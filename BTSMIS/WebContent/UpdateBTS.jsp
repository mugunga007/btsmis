        ...
        <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-header ">
                          <i class="fa fa-edit text-success"></i>  <small>Update BTS Info  <small>(<%=b.getBtsId() %>)</small></small>
                        </h1>
                    </div>
                </div>
                <!-- /. ROW  -->
...
<form class="form-horizontal" action="AddBtsController" method="post">
         ...
         <label for="name" class=" control-label">BTS ID</label>
                ...
                <div class="input-group">
                 <span class="input-group-addon"><span class="glyphicon glyphicon-edit"></span> </span>
                          <input type="text" value="<%=b.getBtsId() %>" name="btsId" readonly="readonly" class="form-control" id="name" placeholder="" required="required"/>
                 </div> 
  ...
  <div class="form-group">
         <label for="name" class=" control-label">BTS Name</label>
                ...
                <div class="input-group">
                 <span class="input-group-addon"><span class="glyphicon glyphicon-edit"></span> </span>
                          <input type="text" value="<%=b.getBtsName() %>" name="btsName"  class="form-control" id="name" placeholder="" required="required"/>
                 </div> 
...
<select name="engineerId"  class="form-control">
                ...
                <option <%if(b.getEngineerId()==pu.getId()){ %> selected="selected" <%} %> value="<%=pu.getId()%>"><%=pu.getFname()%> <%=pu.getLname()%>, <small> <%=is.getRegion(pu.getRegion()).getRname() %> (<%=btm.getBtsByEngineer(pu.getId()).size() %> BTS)</small> </option>
               <%
                     }
               %>
...
SitesClassModel ic = new SitesClassModel();
                 ...
                 for (SitesClass ps : its) {
%>                 
                 
                 
                <option <%if(b.getSitesclass()==ps.getId()){ %> selected="selected" <%} %> value="<%=ps.getId()%>"><%=ps.getName() %> </option>
               <%
                 }
               %>
            </select>
...
SitesCategoryModel ics = new SitesCategoryModel();
                 ...
                 for (SitesCategory pc : isd) {
%>                 
                 
                 
                <option <%if(b.getSitescategory()==pc.getId()){ %> selected="selected" <%} %> value="<%=pc.getId()%>"><%=pc.getName() %> </option>
               <%
                 }
               %>
            </select>
